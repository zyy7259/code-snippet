import ReactDOMInput from 'root/renderers/dom/client/wrapper/ReactDOMInput';
import ReactDOMTextarea from 'root/renderers/dom/client/wrapper/ReactDOMTextarea';
import ReactDOMSelect from 'root/renderers/dom/client/wrapper/ReactDOMSelect';
import ReactDOMOption from 'root/renderers/dom/client/wrapper/ReactDOMOption';
...
import EventPluginRegistry from 'root/renderers/shared/stack/event/EventPluginRegistry';
...
import ReactMultiChild from 'root/renderers/shared/stack/reconciler/ReactMultiChild';
...
import AutoFocusUtils from 'root/renderers/dom/client/wrapper/AutoFocusUtils';
import DOMNamespaces from './DOMNamespaces';
import Flags from './ReactDOMComponentFlags';
...
function enqueuePutListener(inst, registrationName, listener, transaction) {
  ...
  ReactBrowserEventEmitter.listenTo(registrationName, doc);
 ...
 */
class ReactDOMComponent extends ReactMultiChild {
  constructor(element) {
  ...
  _createOpenTagMarkupAndPutListeners(transaction, props) {
    ...
    for (var propKey in props) {
      ...
      if (EventPluginRegistry.registrationNameModules.hasOwnProperty(propKey)) {
        if (propValue) {
  ...
  _updateDOMProperties(lastProps, nextProps, transaction) {
    ...
    for (propKey in lastProps) {
      ...
      if (propKey === STYLE) {
        ...
        EventPluginRegistry.registrationNameModules.hasOwnProperty(propKey)
      ) {
        ...
        if (lastProps[propKey]) {
          ...
          EventPluginHub.deleteListener(this, propKey);
        }
      } else if (isCustomComponent(this._tag, lastProps)) {
        // remove custom component attribute
        if (!RESERVED_PROPS.hasOwnProperty(propKey)) {
    ...
    for (propKey in nextProps) {
      ...
      // don't update if equals last attributes
      if (
        !nextProps.hasOwnProperty(propKey) ||
        nextProp === lastProp ||
      ...
      if (propKey === STYLE) {
        // update style
        // store next prop as previous style
        if (nextProp) {
          nextProp = this._previousStyleCopy = Object.assign({}, nextProp);
        } else {
          this._previousStyleCopy = null;
        }
        // update according to if have previous style
        if (lastProp) {
        ...
        EventPluginRegistryregistrationNameModules.hasOwnProperty(propKey)
      ) {
        // update listener
        // if have new listener, put it
        // if no new listener, and got old listener, delete it
        if (nextProp) {
        ...
        // update custom component attritue
        if (!RESERVED_PROPS.hasOwnProperty(propKey)) {
